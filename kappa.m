function infoKAPPA = kappa(x,w,alpha,dispResults)
% KAPPA
...
%RIZ added return infoKAPPA -- 20080307
...
m(2)=[];
if w==0 || w==-1
    uw=diag(ones(1,m)); %unweighted
%    disp('UNWEIGHTED COHEN''S KAPPA')
%    disp(repmat('-',1,80))
    infoKAPPA = kcomp(x,m,uw,alpha,dispResults);
%    disp(' ')
end
if w==1 || w==-1
    j=repmat((1:1:m),m,1);
    ...
    disp(repmat('-',1,80))
    infoKAPPA = kcomp(x,m,lw,alpha,dispResults);
    disp(' ')
end
if w==2 || w==-1
    j=repmat((1:1:m),m,1);
    ...
    disp(repmat('-',1,80))
    infoKAPPA = kcomp(x,m,qw,alpha,dispResults);
end
return
end

function infoKAPPA = kcomp(x,m,f,alpha,dispResults)
...
p=(1-normcdf(abs(z)))*2;
%results to Return %RIZ
infoKAPPA.po=po;
infoKAPPA.pe=pe;
infoKAPPA.k=k;
infoKAPPA.sek=sek;
infoKAPPA.alpha=alpha;
infoKAPPA.ci=ci;
infoKAPPA.km=km;
infoKAPPA.var=var;
%display results
